...
class QWENWrapper(nn.Module):
    ...
    def forward(self, key_buffer:torch.Tensor, value_buffer:torch.Tensor, 
                input_ids:torch.Tensor, input_mask:torch.Tensor, past_len:int) -> tuple:
        """
        Args:
            ...
            past_len: scalar int
            input_mask: tensor for input mask [batch=1, seq_len]
        
        Returns:
            tuple: (updated_key_buffer, updated_value_buffer, next_token_id)
        """
        
        new_len = input_ids.size(1)
        end_idx = past_len + new_len

        posemb_cos_q = self.cos_rotary_pos_emb.narrow(1, past_len, new_len)
        posemb_sin_q = self.sin_rotary_pos_emb.narrow(1, past_len, new_len)
        ...
        attn_mask = self.attention_mask.narrow(1, 0, new_len).narrow(2, 0, end_idx)
        attn_mask = (attn_mask * input_mask).to(dtype=hidden_states.dtype)
        
        # Clone buffers to avoid in-place modification issues with torch.export
        updated_key_buffer = key_buffer.clone()
        updated_value_buffer = value_buffer.clone()
        ...
        for i, layer in enumerate(self.qwen.model.layers): 
            ...
            # Update KV cache using concatenation (most ONNX-friendly approach)
            # Get buffer size dynamically to avoid constant/dynamic conflicts
            buffer_size_k = key_buffer.size(-1)
            buffer_size_v = value_buffer.size(-2)
            
            # For key: [num_kv_heads, 1, head_dim, past_len] + [num_kv_heads, 1, head_dim, new_len] + remaining
            if past_len > 0:
                k_past = key_buffer[i].narrow(-1, 0, past_len)
                k_remaining = key_buffer[i].narrow(-1, end_idx, buffer_size_k - end_idx)
                updated_key_buffer[i] = torch.cat([k_past, new_k, k_remaining], dim=-1)
            else:
                k_remaining = key_buffer[i].narrow(-1, new_len, buffer_size_k - new_len)
                updated_key_buffer[i] = torch.cat([new_k, k_remaining], dim=-1)
            
            # For value: [num_kv_heads, 1, past_len, head_dim] + [num_kv_heads, 1, new_len, head_dim] + remaining
            if past_len > 0:
                v_past = value_buffer[i].narrow(-2, 0, past_len)
                v_remaining = value_buffer[i].narrow(-2, end_idx, buffer_size_v - end_idx)
                updated_value_buffer[i] = torch.cat([v_past, new_v, v_remaining], dim=-2)
            else:
                v_remaining = value_buffer[i].narrow(-2, new_len, buffer_size_v - new_len)
                updated_value_buffer[i] = torch.cat([new_v, v_remaining], dim=-2)
            
            # Get the active cache for this layer
            k_unique = updated_key_buffer[i].narrow(-1, 0, end_idx)
            v_unique = updated_value_buffer[i].narrow(-2, 0, end_idx)
            ...
            attn = F.softmax(
                torch.matmul(q * posemb_cos_q + rotate_half(q, self.head_dim_half, -1) * posemb_sin_q, k_expanded) + attn_mask, dim=-1, dtype=q.dtype)
        ...
        return updated_key_buffer, updated_value_buffer, tok_id
...
def onnx_export(wrapper: QWENWrapper, output_path:str):
    ...
    # Set this explicitly to avoid the Step 2/3 Mutation Error
    dummy_key_buffer.requires_grad = False
    dummy_value_buffer.requires_grad = False

    dummy_input_ids = torch.zeros(1, 2, dtype=torch.int64, device=device)
    dummy_attention_mask = torch.zeros(1, 2, dtype=torch.int8, device=device)
    dummy_past_len = 0

    inputs = (dummy_key_buffer, dummy_value_buffer, dummy_input_ids, dummy_attention_mask, dummy_past_len)
    input_names = ['key_buffer', 'value_buffer', 'input_ids', 'input_mask', 'past_len']
    output_names = ['updated_key_buffer', 'updated_value_buffer', 'next_token_id']
    
    max_seq_len_dim = torch.export.Dim("max_seq_len", min=2, max=MAX_SEQ_LEN)
    seq_len_dim = torch.export.Dim("seq_len", min=1, max=MAX_SEQ_LEN)

    dynamic_shapes = {
        'key_buffer': {4: max_seq_len_dim},
        'value_buffer': {3: max_seq_len_dim},
        'input_ids': {1: seq_len_dim},
        'input_mask': {1: seq_len_dim},
        'past_len': None,
    }
...
def torch_inference(wrapper: QWENWrapper, tokenizer: AutoTokenizer):
    ...
    # Process prompt
    prompt_tensor = torch.tensor([prompt_ids], dtype=torch.int64, device=device)
    past_len = 0  # No history for first call
    attention_mask = torch.tensor([[0]], dtype=torch.int8, device=device)
    
    print("Processing prompt...")
    with torch.no_grad():
        key_buffer, value_buffer, next_token_tensor = wrapper(
    ...
    for i in range(MAX_NEW_TOKS):
        next_id_tensor = torch.tensor([[next_token_id]], dtype=torch.int64, device=device)
...
def main():
    ...
    with open('/home/shivani/work/llm-inference/configs/config.json', 'r') as f:
        config = json.load(f)
    ...
    out_path = "/home/shivani/work/llm-inference/export3-8B/qwen_wrapper_v2.onnx"
